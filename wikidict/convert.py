...
def get_secondary_formaters() -> list[type[BaseFormat]]:
    ...
    return [BZ2DictFileFormat, DictOrgFormat, MobiFormat, StarDictFormat]
...
def distribute_workload(
    ...
    *,
    ...
    with Pool(processes or len(formaters)) as pool:
        pool.map(
            partial(
                run_formatter,
                locale=locale,
...
def main(locale: str) -> int:
    ...
    # Need to restrict how many jobs are running since Mobi & StarDict formaters can't run concurrently
    distribute_workload(get_secondary_formaters(), *args, processes=2)
    distribute_workload(get_secondary_formaters(), *args, include_etymology=False, processes=2)
