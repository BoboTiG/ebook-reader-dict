...
class DictFileFormat(KoboBaseFormat):
    ...
    def process(self) -> None:
        file = self.output_dir / f"dict-{self.locale}-{self.locale}.df.bz2"
        with bz2.open(file, mode="wt", encoding="utf-8") as fh:
...
class StarDictFormat(DictFileFormat):
    ...
    def _convert(self) -> None:
        ...
        glos.convert(
            inputFilename=str(
                self.output_dir / f"dict-{self.locale}-{self.locale}.df.bz2"
            ),
