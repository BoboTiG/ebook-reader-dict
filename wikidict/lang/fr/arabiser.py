...
def arabiser(texte: str) -> str:    # pragma: no cover
    ...
    for curseur in range(1, len(texte) - 1):
        ...
        if (
            ...
            if diacritiques:
                transcription = transcription + en_arabe["a"]
            transcription = transcription + en_arabe["~"]
...
def appliquer(scheme: str, racine: str, var: str = "") -> str:  # pragma: no cover
    ...
    if (
        var in {"(2)", "(3)", "(5)", "(6)", "(9)"} and racine[1] == "w"
    ):  # ces formes sont régulières, cf Schier p.64. cf Ryding p. 580.
        racine = racine[0] + "W" + racine[2]
    if (
        var in {"(2)", "(3)", "(5)", "(6)", "(9)"} and racine[1] == "y"
    ):  # ces formes sont régulières, cf Schier p.64. cf Ryding p. 580.
        racine = racine[0] + "Y" + racine[2]
